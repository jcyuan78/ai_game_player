...
LOCAL_LOGGER_ENABLE(L"tinyxml", LOGGER_LEVEL_DEBUGINFO);
...
static const wchar_t DOUBLE_QUOTE			= '\"';

// Bunch of unicode info at:
//		http://www.unicode.org/faq/utf_bom.html
//	ef bb bf (Microsoft "lead bytes") - designates UTF-8

static const wchar_t TIXML_UTF_LEAD_0 = 0xefU;
static const wchar_t TIXML_UTF_LEAD_1 = 0xbbU;
static const wchar_t TIXML_UTF_LEAD_2 = 0xbfU;
...
wchar_t* StrPair::ParseWord(wchar_t* p, int strFlags, int* curLineNumPtr)
{
    TIXMLASSERT(p);
//    TIXMLASSERT(endTag && *endTag);
    TIXMLASSERT(curLineNumPtr);
...
wchar_t* XMLDocument::Identify( wchar_t* p, XMLNode** node )
{
    ...
    {   
        ...
        {   //<YUAN>连续出现的<，最后一个之前都作为一般字符处理
            returnNode = CreateUnlinkedNode<XMLText>(_textPool);
            returnNode->_parseLineNum = _parseCurLineNum; // Report line of first non-whitespace character
            wchar_t* ss = p;
            int ii = 0;
            while (p[1] == '<') p++, ii++;
            std::wstring str(ss, ii);
            returnNode->_value.SetStr(str.c_str());
...
void XMLNode::FindAllElements(std::vector<XMLElement*>& out, const std::wstring& name)
{
    ...
    while (first)
    {
        ...
        first->FindAllElements(out, name);
...
bool XMLNode::ParseDeep( wchar_t* & p, StrPair* parentEndTag, int* curLineNumPtr )
{
	...
	if (_document->Error()) return false;
	...
	while( p && *p ) 
    {
       ...
       const int initialLineNum = node->_parseLineNum;
        ...
        StrPair endTag;
        bool br = node->ParseDeep( p, &endTag, curLineNumPtr );
        if ( !br )
        {
            DeleteNode( node );
            if ( !_document->Error() ) 
        ...
        if ( ele ) 
        {
            ...
            {
                if ( parentEndTag ) {   ele->_value.TransferTo( parentEndTag );   }
                node->_memPool->SetTracked();   // created and then immediately deleted.
                DeleteNode( node );
                return true;
            }

            // Handle an end tag returned to this level. And handle a bunch of annoying errors.
            bool mismatch = false;
            if ( endTag.Empty() ) 
            {
                if ( ele->ClosingType() == XMLElement::OPEN ) 
                {   //<YUAN> 没有Close的node, html允许tab不close，直接close
                    LOG_WARNING(L"node:%s missed close tag", ele->Name());
                    //mismatch = true;
                }
            }
            else 
            {
                ...
                {
                    std::wstring str_end = endTag.GetStr();
                    std::transform(str_end.begin(), str_end.end(), str_end.begin(), ::toupper);
...
// --------- XMLText ---------- //
bool XMLText::ParseDeep( wchar_t* &p, StrPair*, int* curLineNumPtr )
{
    if ( this->CData() ) 
    {
...
bool XMLComment::ParseDeep( wchar_t* &p, StrPair*, int* curLineNumPtr )
{
...
bool XMLDeclaration::ParseDeep( wchar_t* &p, StrPair*, int* curLineNumPtr )
{
...
bool XMLUnknown::ParseDeep( wchar_t* &p, StrPair*, int* curLineNumPtr )
{
...
bool XMLAttribute::ParseDeep( wchar_t* &p, bool processEntities, int* curLineNumPtr )
{
    ...
    p = XMLUtil::SkipWhiteSpace( p, curLineNumPtr );
    //<YUAN> 对于HTLM，属性可以不以引号开头，此时必须以空格结尾
    if ( *p != '\"' && *p != '\'' ) 
    {
        p = _value.ParseWord(p, processEntities ? StrPair::ATTRIBUTE_VALUE : StrPair::ATTRIBUTE_VALUE_LEAVE_ENTITIES, curLineNumPtr);
    }
    else
    {
...
std::wstring XMLElement::GetContents(void) const
{
    std::wstring buf;
...
wchar_t* XMLElement::ParseAttributes( wchar_t* p, int* curLineNumPtr )
{
    ...
    while( p ) {
        ...
        if (XMLUtil::IsNameStartChar( ( wchar_t) *p ) ) {
            ...
            bool br = attrib->ParseDeep( p, _document->ProcessEntities(), curLineNumPtr );
            if ( !br)
            {
                LOG_ERROR(L"[err] XML_ERROR_PARSING_ATTRIBUTE, line=%d", *curLineNumPtr);
            ...
            {   // <YUAN> 忽略重复attribute的问题
                LOG_WARNING(L"[err] doubled attribute: name=%s, line=%d", attrib->Name(), *curLineNumPtr);
                DeleteAttribute( attrib );
            }
        }
        // end of the tag
...
//	<ele>foo<b>bar</b></ele>
//
bool XMLElement::ParseDeep( wchar_t* &p, StrPair* parentEndTag, int* curLineNumPtr )
{
    // Read the element name.
    p = XMLUtil::SkipWhiteSpace( p, curLineNumPtr );

    // The closing element is the </element> form. It is parsed just like a regular element then deleted from
    // the DOM.
    if ( *p == '/' ) 
    {
    ...
    bool br = XMLNode::ParseDeep( p, parentEndTag, curLineNumPtr );
...
XMLError XMLDocument::Parse( const wchar_t* p, size_t len )
{
    ...
    memcpy( _charBuffer, p, len*sizeof(wchar_t) );
    _charBuffer[len] = 0;
    ...
    _buffer()
{
    ...
    _restrictedEntityFlag[static_cast< wchar_t>('&')] = true;
    _restrictedEntityFlag[static_cast< wchar_t>('<')] = true;
    _restrictedEntityFlag[static_cast< wchar_t>('>')] = true;	// not required, but consistency is nice
    _buffer.Push( 0 );
}
...
{
    if ( writeBOM ) {
        static const  wchar_t bom[] = { TIXML_UTF_LEAD_0, TIXML_UTF_LEAD_1, TIXML_UTF_LEAD_2, 0 };
